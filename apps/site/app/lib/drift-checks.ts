// ─────────────────────────────────────────────────────────────────────────────
// The canonical drift checks (d01-d12) — ONE implementation, two callers.
//
// WHY THIS MODULE EXISTS
// /api/drift and /api/monitor both publish checks with the SAME IDs (d01-d12),
// and monitor's own route header describes itself as re-running "the 12 drift
// checks". They are therefore two implementations of one contract and must
// agree about any given page. They did not: measured on 2026-09-25 they
// disagreed on 9 of 12 checks, and the cause was that monitor held a VERBATIM
// COPY of drift's PRE-FIX code.
//
// drift had accumulated six documented fixes that monitor never received:
//   1. a closing-brace bound in the value extractor (minified CSS omits the
//      trailing semicolon, so a capture could run into the next rule)
//   2. geometric exclusions in d07 (a 50% circle and a 999px pill are not
//      radius-scale choices and cannot be replaced by a scale token)
//   3. per-component parsing in d04 (a shorthand `padding: 1rem 1.25rem` was
//      counted as one value, dropping the second — measuring syntax variety
//      rather than scale spread)
//   4. token-scale alignment in d04 (a literal landing on a declared --space
//      stop is evidence of a working system, not drift)
//   5. raising thresholds once research showed the old ones flagged standard
//      systems (Tailwind's 10 radius steps, a complete 6-level shadow set)
//   6. a var() exclusion so token REFERENCES are not counted as hardcoded drift
//
// The result was not a marginal difference. On six real cohort sites, offline:
...
// Those readings are published — /contracts/monitor and an MCP tool surface
// them — so the engine was reporting fabricated design drift about other
// people's sites.
//
// WHY EXTRACTION RATHER THAN A PORT
// Porting nine checks by hand is the move that created this defect: drift got
// its fixes, monitor did not, and nothing detected the divergence for months.
// A second copy would drift again the moment either side is touched. With one
// implementation, agreement is structural — there is nothing left to disagree.
//
// drift's own header comments are preserved verbatim below, because they
// document WHY each exclusion exists and are the evidence for the thresholds.
// The route files keep their transport, caching, rate limiting and response
// shaping; only the measurement moved here.
//
// Contract: these functions are PURE — CSS and tokens in, verdict out. No
// fetch, no cache, no clock. Both routes feed them the same inputs.
// ─────────────────────────────────────────────────────────────────────────────

export function extractRootTokens(css: string): Record<string, string> {
...
export function extractVarRefs(css: string): string[] {
  const refs: string[] = [];
...
export function extractValuesByProperty(css: string, props: string[]): string[] {
  const values: string[] = [];
...
export function cleanCssForValueCounting(css: string): string {
  let cleaned = css;
...
export function extractHardcodedColors(css: string): string[] {
...
export function extractHardcodedSpacing(css: string): string[] {
  const cleaned = cleanCssForValueCounting(css);
...
export function checkD01TokenRegistry(tokens: Record<string, string>): CheckResult {
...
export function checkD05ColorVariance(css: string, _tokens: Record<string, string>): CheckResult {
...
export function resolveFamilyToken(
...
export function checkD06FontFamily(css: string, tokens: Record<string, string>): CheckResult {
  const cleaned = cleanCssForValueCounting(css);
...
export function checkD07BorderRadius(css: string): CheckResult {
  const cleaned = cleanCssForValueCounting(css);
...
export function checkD08ShadowVariance(css: string): CheckResult {
  const cleaned = cleanCssForValueCounting(css);
...
export function checkD09TransitionVariance(css: string): CheckResult {
  const cleaned = cleanCssForValueCounting(css);
...
export function checkD10ZIndex(css: string): CheckResult {
  const cleaned = cleanCssForValueCounting(css);
...
/**
 * Run all twelve drift checks against a page's CSS and token table.
 *
 * Both /api/drift and /api/monitor call THIS, so the two engines cannot
 * disagree about a check they share — the shared list IS the agreement.
 * Ordering is the canonical d01..d12.
 *
 * `varRefs` is an explicit parameter rather than being derived here, because
 * the two callers legitimately see different surfaces and that difference is a
 * FIX, not drift:
 *
 *   /api/drift scans stylesheets AND inline `style="..."` attributes, because
 *   React's `style={{}}` renders to an attribute and a token referenced only
 *   from a style object was invisible to d02 and d11. That is how --surface-1
 *   survived: a panel referenced an undeclared token, rendered transparent, and
 *   d02 — the check whose whole job is catching fabricated tokens — reported
 *   PASS for the life of the bug.
 *
...
export function runDriftChecks(
  ...
  return [
    ...
    checkD06FontFamily(css, tokens),
    checkD07BorderRadius(css),
