...
export const reportContract = {
  ...
  source_authority: {
    ...
    composition: `Weighted synthesis of three independent Designesy engines — /score (${ENGINE_CHECK_COUNT}-check audit), /drift (12-check drift radar), /readiness (10-check AI readiness). Each engine is independently validated; report composes their results.`,
  ...
  conformance: {
    ...
    weighting: [
      { dimension: 'score', weight: 0.5, description: `${ENGINE_CHECK_COUNT}-check design-intelligence audit — the broadest quality measure (tokens, typography, motion, accessibility, anti-generic, runtime)` },
  ...
  verification: {
    checks: [
      ...
      { id: 'rp02', item: `Score engine completed — ${ENGINE_CHECK_COUNT}-check audit ran`, pass: 'Score engine returned a result with a score and grade', fail: 'Score engine failed or timed out — the audit did not complete', warn: 'Score engine returned but with SKIP checks — partial result' },
  ...
  relationship_to_core: {
    'designesy.org /score': `Report runs /score as its primary sub-engine (weight 0.5) — the ${ENGINE_CHECK_COUNT}-check audit is the backbone of the composite`,
