...
// ── Category weights (mirrors route.ts CATEGORY_WEIGHTS) ─────────────────────
...
// ── Check definitions ───────────────────────────────────────────────────────
// Static metadata for all 40 checks. Each entry has:
//   id          — check identifier (v01-v41, x01-x03)
//   item        — human-readable check description
//   category    — scoring category
//   type        — 'auto' (deterministic CSS/HTML test) or 'manual' (needs browser)
//   threshold   — what the check measures against (pass/fail boundary)
//   pass        — condition for PASS
//   fail        — condition for FAIL
//   warn        — condition for WARN (partial credit)
//   ceiling     — hard-fail score cap if this check FAILs (null = no ceiling)
//
// Per-check weight = CATEGORY_WEIGHTS[category] / (number of scored checks in
// that category). Manual checks have weight 0 (excluded from scoring).
...
const checksWithWeight: CheckDefinition[] = CHECK_DEFINITIONS.map((c) => {
...
export function GET() {
  return NextResponse.json({
    ...
    totalChecks: checksWithWeight.length,
    scoredChecks: checksWithWeight.filter((c) => c.type === 'auto').length,
    manualChecks: checksWithWeight.filter((c) => c.type === 'manual').length,
    categoryWeights: CATEGORY_WEIGHTS,
    totalWeight: Object.values(CATEGORY_WEIGHTS).reduce((a, b) => a + b, 0),
    checks: checksWithWeight,
