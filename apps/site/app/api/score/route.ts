...
function checkNoAtlasNaming(html: string): CheckResult {
  ...
  const h1 = h1Match?.[1] || '';
  if (title.includes('internal') || h1.includes('internal')) {
    return { status: 'FAIL', detail: `internal found in title/h1: title='${title}', h1='${h1}'` };
  }
  return { status: 'PASS', detail: 'no internal naming in visible title/h1' };
}
