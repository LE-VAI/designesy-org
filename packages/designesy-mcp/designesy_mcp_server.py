...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Vitals, sound toggle, overflow) return SKIP, not FAIL. "
    ...
    },
    {
        ...
        "description": (
            ...
            "Community Group (DTCG) 2025.10 format, returning 10 "
            ...
            "correct dimension units. When NOT to use: for scoring a "
            "whole live site (not just its token file), use "
    ...
    },
    {
        ...
        "description": (
            ...
            "PASS/FAIL/WARN. Use this to verify a motion/animation asset "
            "is well-formed and accessible — required fields (v, fr, ip, "
            "op, w, h, layers), $version, a markers array for "
            "reduced-motion, and no deprecated version. When NOT to use: "
            "for full-site motion scoring (not a single Lottie file), use "
            "designesy_score. Executable — fetches the URL or parses the "
            "raw Lottie JSON, runs 10 checks server-side. No browser "
            "needed. Returns JSON: { checks[{id (m01–m10), name, status "
            "(PASS/FAIL/WARN), detail}], valid, score }. Pass url to "
            "fetch a remote Lottie file, or lottie_file to validate an "
            "inline JSON string. Provide exactly one."
        ),
        "inputSchema": {
            "type": "object",
