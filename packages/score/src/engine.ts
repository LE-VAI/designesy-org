...
function checkNoAtlasNaming(html: string): CheckResult {
  const internalPattern = /\binternal\b/gi;
  const matches = html.match(internalPattern);
  if (!matches || matches.length === 0) return { id: 'v07', item: 'No "internal" naming in public-facing HTML', category: 'identity', status: 'PASS', detail: 'no "internal" text found in HTML' };
  const visible = html.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '');
  const visibleMatches = visible.match(internalPattern);
  if (visibleMatches && visibleMatches.length > 0) return { id: 'v07', item: 'No "internal" naming in public-facing HTML', category: 'identity', status: 'FAIL', detail: `${visibleMatches.length} "internal" reference(s) found in visible HTML` };
  return { id: 'v07', item: 'No "internal" naming in public-facing HTML', category: 'identity', status: 'PASS', detail: `"internal" only in script/style blocks (${matches.length} total)` };
}
